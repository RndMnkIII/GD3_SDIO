...
#define ORIENTACION         0   //0, 1, 2, 3, establece la orientación de trabajo
...
if (FT_811_ENABLE == 1){
   // MODIFICADO POR MI by LIGHTCALAMAR
    ...
    GD.wr32(REG_HSYNC1, 10);      // 41
    GD.wr32(REG_VCYCLE, 525);     // 500
    GD.wr32(REG_VOFFSET, 23);     // 12, 23 actual
    GD.wr32(REG_VSIZE, 480);
    GD.wr32(REG_VSYNC0, 0);
    GD.wr32(REG_VSYNC1, 10);
    GD.wr32(REG_PCLK, 2);
...
#if (FT_810_ENABLE==1)
// Serial.println("FT810");
 cmd_text(GD.w / 2, (GD.h / 2)+(2*(GD.h / 20)), 27, OPT_CENTERX, "FT810 en  linea");
#endif

#if (FT_811_ENABLE==1)
// Serial.println("FT811");
 cmd_text(GD.w / 2, (GD.h / 2)+(2*(GD.h / 20)), 27, OPT_CENTERX, "FT811 en  linea");
#endif

#if (FT_812_ENABLE==1)
// Serial.println("FT812");
 cmd_text(GD.w / 2, (GD.h / 2)+(2*(GD.h / 20)), 27, OPT_CENTERX, "FT812 en linea");
#endif

#if (FT_813_ENABLE==1)
 //Serial.println("FT813");
 cmd_text(GD.w / 2, (GD.h / 2)+(2*(GD.h / 20)), 27, OPT_CENTERX, "FT813 en linea");
#endif

#if (MCU_Teensy3X==0)
 cmd_text(GD.w / 2, (GD.h / 2)+(3*(GD.h / 20)), 27, OPT_CENTERX, "MCU: Arduino");
#endif

#if (MCU_Teensy3X==2)
 cmd_text(GD.w / 2, (GD.h / 2)+(3*(GD.h / 20)), 27, OPT_CENTERX, "MCU: Teensy 3.2");
#endif

#if (MCU_Teensy3X==6)
 cmd_text(GD.w / 2, (GD.h / 2)+(3*(GD.h / 20)), 27, OPT_CENTERX, "MCU: Teensy 3.6");
#endif
...
void GDClass::alert(const char *message)
{
  ...
  ClearColorRGB(0x250000);
  Clear();
  //copy(__bsod, sizeof(__bsod));
  //cmd_text(240, 176, 29, OPT_CENTER, message);

if (ORIENTACION == 0){
   cmd_text(GD.w / 2, (GD.h / 2) - (GD.h / 20), 31, OPT_CENTERX, "ERROR");
   cmd_text(GD.w / 2, (GD.h / 2) + (GD.h / 20), 31, OPT_CENTERX, "Revisar comandos");
  }
...
if (ORIENTACION == 2){
   cmd_text(240, (GD.h / 2) - (GD.h / 20), 31, OPT_CENTERX, "ERROR");
   cmd_text(240, (GD.h / 2) + (GD.h / 20), 31, OPT_CENTERX, "Revisar comandos");
  }
